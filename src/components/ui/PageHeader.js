export default function PageHeader({
  ...
  const baseClass = 'relative overflow-hidden rounded-3xl border border-white/60 bg-white/80 px-6 py-8 shadow-soft backdrop-blur-xl dark:border-slate-800/80 dark:bg-slate-900/60 surface-card';
  return (
    <div className={`${baseClass}${className ? ` ${className}` : ''}`}>
      ...
      <div className="relative z-[1] flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
        <div className="space-y-4">
          {eyebrow && (
            <div className="pill heading-eyebrow w-fit bg-white/80 px-4 py-1 dark:bg-slate-800/70">
              {eyebrow}
            </div>
          )}
          <div>
            <h1 className="font-display text-3xl sm:text-4xl font-semibold text-strong">
              {title}
            </h1>
            {subtitle && (
              <p className="copy-subtle mt-3 max-w-2xl text-soft">
                {subtitle}
              </p>
            )}
          </div>
