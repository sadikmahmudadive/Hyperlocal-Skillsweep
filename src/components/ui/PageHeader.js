export default function PageHeader({
  ...
  const baseClass = 'relative overflow-hidden rounded-3xl border border-soft surface-card px-6 py-8 shadow-soft backdrop-blur-xl';
  return (
    <div className={`${baseClass}${className ? ` ${className}` : ''}`}>
      ...
      <div className="relative z-[1] flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
        <div className="space-y-4">
          {eyebrow && (
            <div className="pill heading-eyebrow w-fit px-4 py-1">
              {eyebrow}
            </div>
          )}
          <div>
